...
@php
    $title = ($t['blog']['title'] ?? $t['nav']['blog'] ?? 'Блог') . ' — Almep Trading';
    ...
    <section class="news-page">
      <div class="news-page__inner">
        ...
        <nav class="breadcrumbs" aria-label="breadcrumb">
          ...
          <span class="breadcrumbs__current" aria-current="page">{{ $t['blog']['title'] ?? 'Блог' }}</span>
        </nav>
        ...
        @if($featured)
          <section class="latest-news">
            <h1 class="latest-news__title">{{ $t['blog']['title'] ?? 'Блог' }}</h1>
            @php
            ...
            <div class="featured-news">
              <a href="/{{ $lang }}/blog/{{ $featured->id }}" class="featured-news__stretched-link" aria-label="{{ $fTitle }}"></a>
              ...
              <div class="featured-news__content">
                ...
                <a href="/{{ $lang }}/blog/{{ $featured->id }}" class="read-more">
                  <span>{{ $t['blog']['learn_more'] ?? '' }}</span><svg width="27" height="20" viewBox="0 0 27 20" fill="none" aria-hidden="true"><path d="M0.013702 10.394L26.1767 9.68691M26.1767 9.68691L16.8428 0.353097M26.1767 9.68691L16.9401 18.9235" stroke="#1C508F"/></svg>
                </a>
              </div>
            </div>
          </section>
        ...
        <section class="all-news">
          @if($page > 1 || !$featured)
            <h2 class="all-news__title">{{ $t['blog']['title'] ?? 'Блог' }}</h2>
          @endif
          ...
          @if(count($grid) === 0 && !$featured)
            <div class="news-page__empty">
              ...
              <p>{{ $t['blog']['no_news'] ?? 'Новости скоро появятся' }}</p>
            </div>
          @else
            <div class="news-grid">
              @foreach($grid as $item)
                ...
                <article class="news-card">
                  <a href="/{{ $lang }}/blog/{{ $item->id }}" class="news-card__stretched-link" aria-label="{{ $iTitle }}"></a>
                  ...
                  <div class="news-card__content">
                    ...
                    <a href="/{{ $lang }}/blog/{{ $item->id }}" class="read-more">
          ...
          @if($blog->hasPages())
            @php $cur = $blog->currentPage(); $last = $blog->lastPage(); @endphp
            <nav class="pagination" aria-label="{{ $t['blog']['aria_pagination'] ?? 'Пагинация' }}">
              <a href="{{ $cur > 1 ? $blog->previousPageUrl() : '#' }}"
                 class="pagination__arrow {{ $cur === 1 ? 'pagination__arrow--disabled' : '' }}"
                 aria-label="{{ $t['blog']['aria_prev'] ?? 'Previous page' }}"
