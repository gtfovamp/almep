    ...
    <section class="article-page">
      <div class="article-page__inner">
        ...
        <nav class="breadcrumbs" aria-label="breadcrumb">
          ...
          <a href="/{{ $lang }}/news" class="breadcrumbs__link">{{ $t['news']['title'] ?? 'Новости' }}</a>
          <svg width="4" height="8" viewBox="0 0 4 8" fill="none" class="breadcrumbs__separator" aria-hidden="true">
              <path d="M1 1L3 4L1 7" stroke="#706F6F" stroke-width="1"/>
          </svg>
          ...
          <aside class="article-page__sidebar">
            ...
            @if($related->isNotEmpty())
              <div class="article-sidebar__block">
                <h2 class="article-sidebar__title">{{ $t['news']['other_news'] ?? 'Другие новости' }}</h2>
                <div class="side-news-list">
                  @foreach($related as $r)
                    ...
                    <article class="side-news-card">
                      <a href="/{{ $lang }}/news/{{ $r->id }}" class="side-news-card__stretched-link" aria-label="{{ $rTitle }}"></a>
                      ...
                      <div class="side-news-card__content">
                        ...
                        <a href="/{{ $lang }}/news/{{ $r->id }}" class="read-more">
            ...
            <div class="article-sidebar__block article-sidebar__subscribe">
              <h2 class="article-sidebar__title article-sidebar__title--center">{{ $t['news']['subscribe_title'] ?? 'Будьте в курсе новостей' }}</h2>
              <p class="article-sidebar__subscribe-text">{{ $t['news']['subscribe_text'] ?? 'Подпишитесь на новости и получайте актуальную информацию первыми' }}</p>
              <form class="subscribe-form" action="#" method="POST">
                @csrf
                <input type="email" name="email" required class="subscribe-form__input" placeholder="{{ $t['news']['email_placeholder'] ?? 'Ваш E-mail' }}" />
                <button type="submit" class="subscribe-form__button">{{ $t['news']['subscribe_button'] ?? 'Подписаться' }}</button>
              </form>
            </div>
...
{{--
    ...
    Route::get('/{lang}/news/{id}', [NewsController::class, 'show'])
        ->where('lang', 'ru|en|az')
        ->name('news.show');
